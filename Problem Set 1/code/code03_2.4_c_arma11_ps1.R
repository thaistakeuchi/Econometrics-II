...
## AR Coefficient -> Y = c + phi*Yt-1 + Et with c = 0 and phi = 0.3
## MA Coefficient -> Y = c + theta*Et-1 + Et with c = 0 and theta = 0.5
phi <- 0.3
theta <- 0.5
c <- 0
...
results <- foreach(
  ...
  for (m in 1:M) {
    # Simulate an ARMA(1,1) process. If g == 1, it is a Gaussian process.
    if (g == 1) {
      Y <- arima.sim(model = list(ar = phi, ma = theta), n = capT, rand.gen = rnorm, mean = c)

      # If g == 0, it is a exponential process
    } else {
      Y <- arima.sim(model = list(ar = phi, ma = theta), n = capT, rand.gen = rexp, mean = c)
    ...
    resultsT$bias[m] <- arma11$coef[3] - c

    # Store the normalized coefficient
    resultsT$normalized_coef[m] <- (arma11$coef[3] - c) / sqrt(arma11$var.coef[3,3])

    # Store the test decision
    resultsT$reject[m] <- as.numeric(
      abs((arma11$coef[3] - c) / sqrt(arma11$var.coef[3,3])) >= qnorm(0.975)
    )
...
if (g == 1) {
  ggsave("figures/figure_convergence-prob-normal_c_arma11.pdf", width = 11, height = 8.5)
} else {
  ggsave("figures/figure_convergence-prob-exp_c_arma11.pdf", width = 11, height = 8.5)
}
...
## Save figure: The figure's file name depends on the value of g
if (g == 1) {
  ggsave("figures/figure_convergence-distr-normal_c_arma11.pdf", width = 11, height = 8.5)
} else {
  ggsave("figures/figure_convergence-distr-exp_c_arma11.pdf", width = 11, height = 8.5)
}
...
if (g == 1) {
  anim_save("frames/gif_convergence-dist-normal_c_arma11.gif")
} else {
  anim_save("frames/gif_convergence-dist-exp_c_arma11.gif")
}
...
# Save figure: The figure's file name depends on the value of g
if (g == 1) {
  ggsave("figures/figure_rejection-rate-normal_c_arma11.pdf", width = 11, height = 8.5)
} else {
  ggsave("figures/figure_rejection-rate-exp_c_arma11.pdf", width = 11, height = 8.5)
}
