...
## 1) AR Coefficient -> Y = c + phi*Yt-1 + Et with c = 0 and phi = 0.3
phi <- 0.3
...
results <- foreach(
  ...
  for (m in 1:M) {
    ...
    resultsT$bias[m] <- ar1$coef[2]

    # Store the normalized coefficient
    resultsT$normalized_coef[m] <- (ar1$coef[2]) / sqrt(ar1$var.coef[2,2])

    # Store the test decision
    resultsT$reject[m] <- as.numeric(
      abs((ar1$coef[2]) / sqrt(ar1$var.coef[2,2])) >= qnorm(0.975)
    )
...
if (g == 1) {
  ggsave("figures/figure_convergence-prob-normal_c_ar1.pdf", width = 11, height = 8.5)
} else {
  ggsave("figures/figure_convergence-prob-exp_c_ar1.pdf", width = 11, height = 8.5)
}
...
## Save figure: The figure's file name depends on the value of g
if (g == 1) {
  ggsave("figures/figure_convergence-distr-normal_c_ar1.pdf", width = 11, height = 8.5)
} else {
  ggsave("figures/figure_convergence-distr-exp_c_ar1.pdf", width = 11, height = 8.5)
}
...
if (g == 1) {
  anim_save("frames/gif_convergence-dist-normal_c_ar1.gif")
} else {
  anim_save("frames/gif_convergence-dist-exp_c_ar1.gif")
}
...
# Save figure: The figure's file name depends on the value of g
if (g == 1) {
  ggsave("figures/figure_rejection-rate-normal_c_ar1.pdf", width = 11, height = 8.5)
} else {
  ggsave("figures/figure_rejection-rate-exp_c_ar1.pdf", width = 11, height = 8.5)
}
